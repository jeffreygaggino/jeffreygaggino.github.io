...
If this repo ever grows into several independent contexts, the layout becomes multi-context: a root `CONTEXT-MAP.md` pointing at one `CONTEXT.md` per context under `src/<context>/`, each with its own `docs/adr/` for context-scoped decisions, and the root `docs/adr/` reserved for system-wide ones. Until `CONTEXT-MAP.md` exists at the root, treat this repo as single-context.
